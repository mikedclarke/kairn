# gpui-terminal (vendored fork)

Vendored from [zortax/gpui-terminal](https://github.com/zortax/gpui-terminal) at
rev `51f0292`, MIT OR Apache-2.0 (licenses in this directory). Local changes on
top of upstream:

- Scroll wheel support (`TerminalView::on_scroll`): mouse-wheel reports when
  the application has mouse tracking enabled, arrow keys on the alternate
  screen, local scrollback otherwise; typing snaps back to the live view.
- The renderer honours `display_offset`, so scrollback is actually visible
  while scrolled, and the cursor shifts/hides with the scrolled viewport.

Worth offering upstream once battle-tested.
