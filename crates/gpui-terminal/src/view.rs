...
pub struct TerminalView {
    ...
    /// Layout facts from the last paint (content origin, cell width, cell
    /// height), for mapping mouse positions to grid cells outside the paint
    /// closure
    last_layout: Arc<parking_lot::Mutex<Option<(Point<Pixels>, Pixels, Pixels)>>>,
...
impl TerminalView {
    ...
    ///
    /// Wheel/trackpad deltas accumulate into whole lines. Depending on the
    /// terminal mode the lines either go to the application (mouse reports
    /// for TUIs with mouse tracking, arrow keys on the alternate screen) or
    ...
    fn on_scroll(
        ...
        let Some((origin, cell_width, cell_height)) = *self.last_layout.lock() else {
            return;
        };
        let delta_y: f32 = event.delta.pixel_delta(cell_height).y.into();
        let cell_height_f32: f32 = cell_height.into();
        self.scroll_accum += delta_y / cell_height_f32;
        ...
        if mouse_mode || mode.contains(TermMode::ALT_SCREEN) {
            let point =
                crate::mouse::pixel_to_cell(event.position, origin, cell_width, cell_height);
            if let Some(bytes) = crate::mouse::scroll_report(lines, point, 0, mode) {
                let mut writer = self.stdin_writer.lock();
                // A mouse report encodes one wheel click; repeat it per line.
                // The arrow-key fallback already encodes the repetition.
                let repeats = if mouse_mode { lines.unsigned_abs() } else { 1 };
                for _ in 0..repeats {
                    let _ = writer.write_all(&bytes);
                }
                let _ = writer.flush();
            ...
            .child(
                canvas(
                    ...
                    move |bounds, _, window, cx| {
                        ...
                        // mouse handlers can map positions to grid cells.
                        *last_layout.lock() = Some((
                            Point {
