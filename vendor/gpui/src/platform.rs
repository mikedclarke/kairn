...
pub(crate) trait PlatformWindow: HasWindowHandle + HasDisplayHandle {
    ...
    /// Tells the platform that the window is dirty. A backend that parks its frame loop
    /// when nothing needs drawing has to be woken here, or the window never redraws.
