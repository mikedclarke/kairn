...
impl LineWrapper {
                ...
                let item_width = match candidate {
                    WrapBoundaryCandidate::Char { character: c } => {
                        ...
                        if Self::is_word_char(c) {
                            ...
                            // KAIRN: never break directly before closing punctuation,
                            // which would strand it at the start of the next line.
                            if c != ' '
    ...
    /// KAIRN: closing/trailing punctuation a wrapped line must not start
    /// with. A break candidate directly before these would push them onto
    /// the next line alone (`off` / `),`), so none is recorded there; the
    /// preceding word travels with its punctuation instead. (`,`/`.`/`:`
    /// and friends are already word characters above and never split from
    /// the word they trail.)
    pub(crate) fn is_no_break_before(c: char) -> bool {
        matches!(
            c,
            ')' | ']'
                | '}'
